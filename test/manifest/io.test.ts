...
import { SkmError } from "../../src/shared/errors.js";
import { initLockfile, initManifest } from "../../src/manifest/init.js";
import {
  readLockfile,
  readManifest,
  readManifestResult,
...
import { mergeSkillState } from "../../src/manifest/merge.js";
import { DEFAULT_OUTPUT_DIR } from "../../src/manifest/types.js";
...
test("readManifest rejects malformed nested skill entries from disk", async () => {
  const root = await createTempDir("skm-manifest-");
  const manifestPath = path.join(root, "skills.json");
  await mkdir(root, { recursive: true });
...
test("readLockfile rejects malformed nested skill entries from disk", async () => {
  const root = await createTempDir("skm-manifest-");
  const lockfilePath = path.join(root, "skills.lock.json");
  await mkdir(root, { recursive: true });
...
test("mergeSkillState combines manifest intent with lockfile state", () => {
